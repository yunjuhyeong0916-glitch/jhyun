...
module TX_PAM_FIR_TOP_FLAT_128B (
    input  wire         rstb,
    input  wire         i_clk,
    input  wire         ffe_en,

    // ------------------------------------------------------------
    // PRBS / external pattern (128b 직접 입력)
    // ------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    // 8tap × 8bit = 64bit flat coefficients
    // ------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    // TX core (PRBS128 VERSION)
    // ------------------------------------------------------------
    TX_PAM4_FIR_TOP_128B u_tx (
        .rstb        (rstb),
        .i_clk       (i_clk),
        .ffe_en      (ffe_en),
