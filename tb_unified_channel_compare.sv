...
module tb_unified_channel_compare;
  ...
  task automatic print_phase_summary(
    ...
    begin
      ...
      for (li = 1; li < NUM_LAGS; li = li + 1) begin
        if (err_unified_lag[li] < best_err_unified) begin
          best_err_unified = err_unified_lag[li];
          best_li_unified = li;
        end
      end
      ...
      theo_cand_unified_max = theo_final_candidates_per_lane_max(mode_idx, 3);
      ...
      $display("Actual avg candidates/lane/input_symbol  unified=%0.3f", cand_avg_unified);
      $display("Candidates/lane/symbol(base) unified=%0d", theo_cand_unified);
      $display("Candidates/lane/symbol(max)  unified=%0d", theo_cand_unified_max);
      $display("Expand hits(ps/ns)      unified=%0d/%0d",
               ps_expand_acc_unified_phase, ns_expand_acc_unified_phase);
      $display("Expand rate(ps/ns)      unified=%0.3f/%0.3f",
               ps_expand_rate_unified, ns_expand_rate_unified);
      $display("preBM/lane/symbol       unified=%0d", prebm_per_lane_unified);
      $display("[Per-lane cumulative complexity over %0d input symbols]", per_lane_input_syms);
      $display("  FINAL BM/ACS   unified=%0d", cand_acc_unified_phase / LANES);
      $display("  pre-BM         unified=%0d", prebm_total_unified / LANES);
      $display("  BM-like total  unified=%0d", bm_total_unified / LANES);
      $display("  KSEL(max est.) unified=%0d", ksel_total_unified / LANES);
      $display("  MEMW           unified=%0d", memw_total_unified / LANES);
      $display("[All-lanes cumulative complexity]");
      $display("  FINAL BM/ACS   unified=%0d", cand_acc_unified_phase);
      $display("  pre-BM         unified=%0d", prebm_total_unified);
      $display("  BM-like total  unified=%0d", bm_total_unified);
      $display("  KSEL(max est.) unified=%0d", ksel_total_unified);
      $display("  MEMW           unified=%0d", memw_total_unified);
    end
  endtask
        ...
        if (count_en) begin
          ...
          for (li = 0; li < NUM_LAGS; li = li + 1) begin
            ...
            for (ln = 0; ln < LANES; ln = ln + 1) begin
              ...
              if ((y_unified !== expv) && (y_unified !== -expv))
                beat_err_unified = beat_err_unified + 1;
            end
  ...
  initial begin : STIM
    ...
    $display("========================================");
    $display("Unified MLSD channel sweep completed");
